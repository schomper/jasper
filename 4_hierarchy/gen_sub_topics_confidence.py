...
# ------------------------------------------------------------------------------
# Name: gen_sub_topics_confidence.py
#
# Usage: Using the gamma file and the format file this script produces a format
#        file for each one of the sub topics. This is based of the
#        highest value topic for a document with a confidence interval
# ------------------------------------------------------------------------------

import sys
# TODO: Actually implement confidence interval
...
def main():
    if len(sys.argv) != 6:
        print('./gen_sub_topics_highest.py <gamma_file> <format_file> ' +
              '<confidence> <num_topics> <output_string>')
        exit(1)
